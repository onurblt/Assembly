			...
			*/
			else if (click)
			{
				...
				if (coord.mLine + 1 < mLines.size())
				{
					...
					if (coord.mColumn == 0 && !mLines[coord.mLine + 1].empty())
					{
						if (mBreakpoints.find(coord.mLine + 1) != mBreakpoints.end())
						{
							mBreakpoints.erase(coord.mLine + 1);
						}
						else
						{
							mBreakpoints.insert(coord.mLine + 1);
						}
...
void TextEditor::Render()
{
	...
	char buf[16];
#ifdef _WIN32
	_snprintf(buf, 16, " %d ", globalLineMax);
#else
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::Assembly()
{
	static bool inited = false;
	static LanguageDefinition langDef;
	if (!inited)
